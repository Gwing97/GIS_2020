...
MapControl.prototype.ShowPoints = function (data) {
	var me = this;
	var points = Array();
	...
	for (var i in data) {
		points.push(new ol.Feature({
			id: data[i][0],
			geometry: format.readGeometry(data[i][1], {
				dataProjection: 'EPSG:4326',
				featureProjection: 'EPSG:3857'
			})
		}))
	}
	me._Map.removeLayer(me.VectorLayer);
	me.VectorSource = new ol.source.Vector({
		features: points
	});
	me.VectorLayer.setSource(me.VectorSource);
	me._Map.addLayer(me.VectorLayer);
}

MapControl.prototype.GotoPoint = function (id, isOnView = false) {
	var me = this;
	Connector({
		params: [{
				type: "IMAGE_GETLOCATION(ID)",
				data: {
					"id": id
				}
			}, {
		...
		success: function (json) {
			for (var i in json.results) {
				...
				if (result.success) {
					if (result.message === "IMAGE_GETLOCATION(ID)") {
						var format = new ol.format.WKT();
						var point = format.readGeometry(result.data[1], {
							dataProjection: 'EPSG:4326',
							featureProjection: 'EPSG:3857'
						});
						var coordinate = point.getCoordinates();
						me.ShowPopup(id, coordinate);
						if (!isOnView) {
							var my_view = new ol.View({
								center: coordinate,
								zoom: 15
							});
							me._Map.setView(my_view);
						}
					} else if (result.message === "IMAGE_GETINFO(ID)") {
						ShowImageInfo(result.data)
					} else if (result.message === "IMAGE_GETTAG(ID)") {
						ShowImageTag(result.data);
					}
				} else {
